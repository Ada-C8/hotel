...
describe "Hotel" do
  ...
  describe "#initialize" do
    it "@rooms should be kind of hash" do
      @two_room_hotel.rooms.must_be_kind_of Hash
    end
    it "Should return an array of the rigth length" do
      @two_room_hotel.rooms.length.must_equal 2
    end
    it "Each room should be kind of integer" do
      @two_room_hotel.rooms.keys[0].must_be_kind_of Integer
    end
    it "All_reservations must be kind of array" do
      @two_room_hotel.all_reservations.must_be_kind_of Array
    end
    it "All_reservations must be empty if no reservations were made" do
      @two_room_hotel.all_reservations.length.must_equal 0
    end
    it "All_blocks must be kind of array" do
      @two_room_hotel.all_blocks.must_be_kind_of Array
    end
    it "All_blocks must be empty if no blocks were created" do
      @two_room_hotel.all_blocks.length.must_equal 0
    end
  end

  describe "#room_unavailable" do
    it "Returns an array of dates for given room number" do
      @two_room_hotel.room_unavailable(1).must_be_kind_of Array
    end
    it "Returns empty array if no reservations were made for given room number" do
      @two_room_hotel.room_unavailable(1).length.must_equal 0
    end
    it "Returns an array of one element if one reservation was made for given room number" do
      new_reservation = @two_room_hotel.make_reservation(@date_range, 1)
      @two_room_hotel.room_unavailable(1).length.must_equal @date_range.dates_within_range.length
    end
    it "Returns an array of dates if one block was created" do
      new_block = @five_room_hotel.create_block(@date_range, 3)
      @five_room_hotel.room_unavailable(1).must_equal @date_range.dates_within_range
    end
  end

  describe "#make_reservation" do
    it "Returns an instance of class reservation" do
      @two_room_hotel.make_reservation(@date_range, 1).must_be_kind_of BookingSystem::Reservation
    end
    it "Adds new reservation to all_reservations array" do
      first_reservation = @two_room_hotel.make_reservation(@date_range, 1)
      @two_room_hotel.all_reservations.length.must_equal 1
    end
  ...
  describe "#list_of_reservations" do
    ...
    it "Returns an array of the right length" do
      @two_room_hotel.list_of_reservations(@date).length.must_equal 2
    end
  end

  describe "#list_of_available_rooms" do
    it "Returns an instanse of hash" do
      @two_room_hotel.list_of_available_rooms(@date_range).must_be_kind_of Hash
    end
    it "Returns the right list of available rooms if 1st room was booked" do
      reservation = @two_room_hotel.make_reservation(@date_range, 1)
      @two_room_hotel.list_of_available_rooms(@date_range).must_equal ({2 => 200})
    end
  ...
  describe "#create_block" do
    before do
      @new_block = @eight_room_hotel.create_block(@date_range, 2)
      @another_block = @eight_room_hotel.create_block(@date_range, 7)
    end
    ...
    it "Raise an error if no rooms available for given date range" do
      proc { @eight_room_hotel.create_block(@date_range, 3) }.must_raise BookingSystem::NoRoomAvailableError
    end
  end
  ...
  describe "#make_reservation_from_block" do
    before do
      @new_block = @five_room_hotel.create_block(@date_range, 3)
      @reservation_from_block = @five_room_hotel.make_reservation_from_block(@new_block, 1)
    end
    it "Returns an instance of class Reservation" do
  ...
  describe "Check total cost for two types of reservations" do
    before do
      @block = @five_room_hotel.create_block(@date_range, 3)
      @general_reservation = @five_room_hotel.make_reservation(@date_range, 4)
      @reservation_from_block = @five_room_hotel.make_reservation_from_block(@block, 1)
    end
